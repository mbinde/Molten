...
struct ServiceCoordinationTests {
    ...
    @Test("Should handle catalog item updates affecting inventory references")
    func testCatalogUpdateInventoryReferences() async throws {
        let (catalogService, inventoryService) = createMockServices()
        ...
        // Update catalog item (in a real system, this might affect inventory)
        var updatedCatalogItem = savedCatalogItem
        updatedCatalogItem = CatalogItemModel(
            id: updatedCatalogItem.id,
            name: "Updated Name",
            code: updatedCatalogItem.code,
            manufacturer: updatedCatalogItem.manufacturer
        )
    ...
    @Test("Should handle catalog item deletion with inventory references")
    func testCatalogDeletionInventoryReferences() async throws {
        let (catalogService, inventoryService) = createMockServices()
        ...
        // Attempt to delete catalog item that has inventory references
        // The business logic should determine what happens:
        // - Allow deletion (leaving orphaned inventory)
        // - Prevent deletion 
        // - Cascade delete inventory items
        
        do {
            try await catalogService.deleteItem(withId: savedCatalogItem.id)
            
            // If deletion is allowed, verify final state
            let remainingCatalogItems = try await catalogService.getAllItems()
            let remainingInventoryItems = try await inventoryService.getAllItems()
            
            #expect(remainingCatalogItems.isEmpty, "Catalog item should be deleted")
            
            // Inventory behavior depends on business rules
            // Test that the system is in a consistent state either way
            if remainingInventoryItems.isEmpty {
                // Cascade delete occurred
                #expect(true, "Cascade delete is valid business logic")
            } else {
                // Orphaned inventory items remain
                #expect(remainingInventoryItems.first?.catalogCode == savedInventoryItem.catalogCode, "Orphaned inventory should maintain catalog code")
            }
            
        } catch {
            // Deletion prevented due to references - also valid business logic
            let remainingCatalogItems = try await catalogService.getAllItems()
            let remainingInventoryItems = try await inventoryService.getAllItems()
            
            #expect(remainingCatalogItems.count == 1, "Catalog item should remain when deletion is prevented")
            #expect(remainingInventoryItems.count == 1, "Inventory item should remain when catalog deletion is prevented")
        }
    }
    ...
    func testCompleteWorkflowCoordination() async throws {
        ...
        for var inventoryItem in inventoryItems {
            inventoryItem.quantity += 5 // Simulate restocking
            _ = try await inventoryService.updateItem(inventoryItem)
        }
