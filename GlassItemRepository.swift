...
/// Domain model representing a glass item
struct GlassItemModel {
...
// MARK: - GlassItemModel Extensions

extension GlassItemModel: Equatable {
    static func == (lhs: GlassItemModel, rhs: GlassItemModel) -> Bool {
        return lhs.naturalKey == rhs.naturalKey
    }
}

extension GlassItemModel: Hashable {
    func hash(into hasher: inout Hasher) {
        hasher.combine(naturalKey)
    }
}

extension GlassItemModel: Identifiable {
    var id: String { naturalKey }
}

// MARK: - Natural Key Helper
...
extension GlassItemModel {
    ...
    ///   - manufacturer: Manufacturer identifier
    ///   - sku: Manufacturer SKU
    ///   - sequence: Sequence number (typically 0 for first item)
    /// - Returns: Formatted natural key string
    static func createNaturalKey(manufacturer: String, sku: String, sequence: Int = 0) -> String {
        return "\(manufacturer)-\(sku)-\(sequence)"
    }
}