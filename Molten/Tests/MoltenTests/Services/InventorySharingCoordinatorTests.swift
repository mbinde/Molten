...
struct InventorySharingCoordinatorTests {
    ...
    func testCreateShareFromInventory() async throws {
        ...
        let shareCode = try await coordinator.shareMyInventory(items: [item])
    ...
    func testFilterZeroQuantity() async throws {
        ...
        _ = try await coordinator.shareMyInventory(items: [item1, item2])
    ...
    func testUpdateShare() async throws {
        ...
        try await coordinator.updateMyShare(shareCode: "A7B2X9", items: [item])
...
class MockInventorySharingService: InventorySharingService {
    ...
    override func createShare(items: [InventoryItemSnapshot]) async throws -> String {
    ...
    override func updateShare(shareCode: String, items: [InventoryItemSnapshot]) async throws {
        updateCalled = true
        lastShareCode = shareCode
        lastItems = items
    }
