...
class TestFixesMockGlassItemRepository: @unchecked Sendable, GlassItemRepository {
    ...
    nonisolated func createItem(_ item: GlassItemModel) async throws -> GlassItemModel {
    ...
    nonisolated func createItems(_ items: [GlassItemModel]) async throws -> [GlassItemModel] {
    ...
    nonisolated func fetchItems(matching predicate: NSPredicate?) async throws -> [GlassItemModel] {
        return items
    }

    nonisolated func fetchItems(byManufacturer manufacturer: String) async throws -> [GlassItemModel] {
        return items.filter { $0.manufacturer == manufacturer }
    }

    nonisolated func searchItems(text: String) async throws -> [GlassItemModel] {
    ...
    nonisolated func updateItem(_ item: GlassItemModel) async throws -> GlassItemModel {
    ...
    nonisolated func deleteItem(naturalKey: String) async throws {
        items.removeAll { $0.natural_key == naturalKey }
    }

    nonisolated func deleteItems(naturalKeys: [String]) async throws {
        items.removeAll { naturalKeys.contains($0.natural_key) }
    }

    nonisolated func naturalKeyExists(_ naturalKey: String) async throws -> Bool {
        return items.contains { $0.natural_key == naturalKey }
    }

    nonisolated func generateNextNaturalKey(manufacturer: String, sku: String) async throws -> String {
    ...
    nonisolated func fetchItem(byNaturalKey naturalKey: String) async throws -> GlassItemModel? {
        return items.first { $0.natural_key == naturalKey }
    }

    nonisolated func fetchItems(byCOE coe: Int32) async throws -> [GlassItemModel] {
        return items.filter { $0.coe == coe }
    }

    nonisolated func fetchItems(byStatus status: String) async throws -> [GlassItemModel] {
        return items.filter { $0.mfr_status == status }
    }

    nonisolated func getDistinctManufacturers() async throws -> [String] {
        return Array(Set(items.map { $0.manufacturer })).sorted()
    }

    nonisolated func getDistinctCOEValues() async throws -> [Int32] {
        return Array(Set(items.map { $0.coe })).sorted()
    }

    nonisolated func getDistinctStatuses() async throws -> [String] {
...
class TestFixesMockInventoryRepository: @unchecked Sendable, InventoryRepository {
    ...
    nonisolated func fetchInventory(matching predicate: NSPredicate?) async throws -> [InventoryModel] {
        return items
    }

    nonisolated func fetchInventory(byId id: UUID) async throws -> InventoryModel? {
        return items.first { $0.id == id }
    }

    nonisolated func fetchInventory(forItem item_natural_key: String) async throws -> [InventoryModel] {
        return items.filter { $0.item_natural_key == item_natural_key }
    }

    nonisolated func fetchInventory(forItem item_natural_key: String, type: String) async throws -> [InventoryModel] {
        return items.filter { $0.item_natural_key == item_natural_key && $0.type == type }
    }

    nonisolated func createInventory(_ inventory: InventoryModel) async throws -> InventoryModel {
        items.append(inventory)
        return inventory
    }

    nonisolated func createInventories(_ inventories: [InventoryModel]) async throws -> [InventoryModel] {
        items.append(contentsOf: inventories)
        return inventories
    }

    nonisolated func updateInventory(_ inventory: InventoryModel) async throws -> InventoryModel {
    ...
    nonisolated func deleteInventory(id: UUID) async throws {
        items.removeAll { $0.id == id }
    }

    nonisolated func deleteInventory(forItem item_natural_key: String) async throws {
        items.removeAll { $0.item_natural_key == item_natural_key }
    }

    nonisolated func deleteInventory(forItem item_natural_key: String, type: String) async throws {
        items.removeAll { $0.item_natural_key == item_natural_key && $0.type == type }
    }

    nonisolated func getTotalQuantity(forItem item_natural_key: String) async throws -> Double {
        return items.filter { $0.item_natural_key == item_natural_key }.reduce(0) { $0 + $1.quantity }
    }

    nonisolated func getTotalQuantity(forItem item_natural_key: String, type: String) async throws -> Double {
        return items.filter { $0.item_natural_key == item_natural_key && $0.type == type }.reduce(0) { $0 + $1.quantity }
    }

    nonisolated func addQuantity(_ quantity: Double, toItem item_natural_key: String, type: String) async throws -> InventoryModel {
    ...
    nonisolated func subtractQuantity(_ quantity: Double, fromItem item_natural_key: String, type: String) async throws -> InventoryModel? {
    ...
    nonisolated func setQuantity(_ quantity: Double, forItem item_natural_key: String, type: String) async throws -> InventoryModel? {
        if let existingIndex = items.firstIndex(where: { $0.item_natural_key == item_natural_key && $0.type == type }) {
            if quantity <= 0 {
                items.remove(at: existingIndex)
                return nil
    ...
    nonisolated func getDistinctTypes() async throws -> [String] {
        return Array(Set(items.map { $0.type })).sorted()
    }
    
    nonisolated func getItemsWithInventory() async throws -> [String] {
        return Array(Set(items.map { $0.item_natural_key })).sorted()
    }
    
    nonisolated func getItemsWithInventory(ofType type: String) async throws -> [String] {
        return Array(Set(items.filter { $0.type == type }.map { $0.item_natural_key })).sorted()
    }
    
    nonisolated func getItemsWithLowInventory(threshold: Double) async throws -> [(item_natural_key: String, type: String, quantity: Double)] {
        return items
    ...
    nonisolated func getItemsWithZeroInventory() async throws -> [String] {
        // This would typically track items that previously had inventory
        return []
    }
    
    nonisolated func getInventorySummary() async throws -> [InventorySummaryModel] {
    ...
    nonisolated func getInventorySummary(forItem item_natural_key: String) async throws -> InventorySummaryModel? {
    ...
    nonisolated func estimateInventoryValue(defaultPricePerUnit: Double) async throws -> [String: Double] {
        let grouped = Dictionary(grouping: items, by: { $0.item_natural_key })
...
class TestFixesMockLocationRepository: @unchecked Sendable, LocationRepository {
    ...
    nonisolated func fetchLocations(matching predicate: NSPredicate?) async throws -> [LocationModel] {
        return locations
    }
    
    nonisolated func fetchLocations(forInventory inventory_id: UUID) async throws -> [LocationModel] {
        return locations.filter { $0.inventory_id == inventory_id }
    }
    
    nonisolated func fetchLocations(withName locationName: String) async throws -> [LocationModel] {
        return locations.filter { $0.location == locationName }
    }
    
    nonisolated func createLocation(_ location: LocationModel) async throws -> LocationModel {
        locations.append(location)
        return location
    }
    
    nonisolated func createLocations(_ locations: [LocationModel]) async throws -> [LocationModel] {
        self.locations.append(contentsOf: locations)
        return locations
    }
    
    nonisolated func updateLocation(_ location: LocationModel) async throws -> LocationModel {
    ...
    nonisolated func deleteLocation(_ location: LocationModel) async throws {
        locations.removeAll { $0.id == location.id }
    }
    
    nonisolated func deleteLocations(forInventory inventory_id: UUID) async throws {
        locations.removeAll { $0.inventory_id == inventory_id }
    }
    
    nonisolated func deleteLocations(withName locationName: String) async throws {
        locations.removeAll { $0.location == locationName }
    }
    
    nonisolated func setLocations(_ locations: [(location: String, quantity: Double)], forInventory inventory_id: UUID) async throws {
        // Remove existing locations for this inventory
        self.locations.removeAll { $0.inventory_id == inventory_id }
    ...
    nonisolated func addQuantity(_ quantity: Double, toLocation locationName: String, forInventory inventory_id: UUID) async throws -> LocationModel {
        if let existingIndex = locations.firstIndex(where: { $0.inventory_id == inventory_id && $0.location == locationName }) {
    ...
    nonisolated func subtractQuantity(_ quantity: Double, fromLocation locationName: String, forInventory inventory_id: UUID) async throws -> LocationModel? {
        guard let existingIndex = locations.firstIndex(where: { $0.inventory_id == inventory_id && $0.location == locationName }) else {
            throw RepositoryError.itemNotFound
        }
    ...
    nonisolated func moveQuantity(_ quantity: Double, fromLocation: String, toLocation: String, forInventory inventory_id: UUID) async throws {
    ...
    nonisolated func getDistinctLocationNames() async throws -> [String] {
        return Array(Set(locations.map { $0.location })).sorted()
    }
    
    nonisolated func getLocationNames(withPrefix prefix: String) async throws -> [String] {
        let distinctNames = try await getDistinctLocationNames()
        return distinctNames.filter { $0.hasPrefix(prefix) }
    }
    
    nonisolated func getInventoriesInLocation(_ locationName: String) async throws -> [UUID] {
        return Array(Set(locations.filter { $0.location == locationName }.map { $0.inventory_id }))
    }
    
    nonisolated func getLocationUtilization() async throws -> [String: Double] {
    ...
    nonisolated func getLocationUsageCounts() async throws -> [(location: String, usageCount: Int)] {
        let grouped = Dictionary(grouping: locations, by: { $0.location })
        return grouped.map { (location: $0.key, usageCount: $0.value.count) }
            .sorted { $0.location < $1.location }
    }
    
    nonisolated func validateLocationQuantities(forInventory inventory_id: UUID, expectedTotal: Double) async throws -> Bool {
    ...
    nonisolated func getLocationQuantityDiscrepancy(forInventory inventory_id: UUID, expectedTotal: Double) async throws -> Double {
        let actualTotal = locations
            .filter { $0.inventory_id == inventory_id }
            .reduce(0) { $0 + $1.quantity }
    ...
    nonisolated func findOrphanedLocations() async throws -> [LocationModel] {
...
class TestFixesMockItemTagsRepository: @unchecked Sendable, ItemTagsRepository {
    ...
    nonisolated func fetchTags(forItem item_natural_key: String) async throws -> [String] {
        return itemTags[item_natural_key] ?? []
    }

    nonisolated func fetchTagsForItems(_ itemNaturalKeys: [String]) async throws -> [String: [String]] {
        var result: [String: [String]] = [:]
        for key in itemNaturalKeys {
            if let tags = itemTags[key], !tags.isEmpty {
                result[key] = tags
            }
        }
        return result
    ...
    nonisolated func addTag(_ tag: String, toItem item_natural_key: String) async throws {
    ...
    nonisolated func addTags(_ tags: [String], toItem item_natural_key: String) async throws {
        var currentTags = itemTags[item_natural_key] ?? []
        for tag in tags {
            if !currentTags.contains(tag) {
                currentTags.append(tag)
    ...
    nonisolated func removeTag(_ tag: String, fromItem item_natural_key: String) async throws {
        var currentTags = itemTags[item_natural_key] ?? []
        currentTags.removeAll { $0 == tag }
        itemTags[item_natural_key] = currentTags
    }
    
    nonisolated func removeAllTags(fromItem item_natural_key: String) async throws {
        itemTags.removeValue(forKey: item_natural_key)
    }
    
    nonisolated func setTags(_ tags: [String], forItem item_natural_key: String) async throws {
        itemTags[item_natural_key] = tags
    }
    
    nonisolated func getAllTags() async throws -> [String] {
        return Array(Set(itemTags.values.flatMap { $0 })).sorted()
    }
    
    nonisolated func getTags(withPrefix prefix: String) async throws -> [String] {
        let allTags = try await getAllTags()
        return allTags.filter { $0.hasPrefix(prefix) }
    }
    
    nonisolated func getMostUsedTags(limit: Int) async throws -> [String] {
    ...
    nonisolated func fetchItems(withTag tag: String) async throws -> [String] {
        return itemTags.compactMap { (naturalKey, tags) in
            tags.contains(tag) ? naturalKey : nil
        }
    }
    
    nonisolated func fetchItems(withAllTags tags: [String]) async throws -> [String] {
        return itemTags.compactMap { (naturalKey, itemTagList) in
            tags.allSatisfy { itemTagList.contains($0) } ? naturalKey : nil
        }
    }
    
    nonisolated func fetchItems(withAnyTags tags: [String]) async throws -> [String] {
        return itemTags.compactMap { (naturalKey, itemTagList) in
            tags.contains { itemTagList.contains($0) } ? naturalKey : nil
        }
    }
    
    nonisolated func getTagUsageCounts() async throws -> [String: Int] {
        return Dictionary(grouping: itemTags.values.flatMap { $0 }, by: { $0 })
            .mapValues { $0.count }
    }
    
    nonisolated func getTagsWithCounts(minCount: Int) async throws -> [(tag: String, count: Int)] {
        let tagCounts = Dictionary(grouping: itemTags.values.flatMap { $0 }, by: { $0 })
            .mapValues { $0.count }
    ...
    nonisolated func tagExists(_ tag: String) async throws -> Bool {
...
class TestFixesMockItemMinimumRepository: ItemMinimumRepository {
    ...
    nonisolated func fetchMinimums(matching predicate: NSPredicate?) async throws -> [ItemMinimumModel] {
        return Array(minimums.values)
    }
    
    nonisolated func fetchMinimum(forItem item_natural_key: String, type: String) async throws -> ItemMinimumModel? {
        let key = "\(item_natural_key)-\(type)"
        return minimums[key]
    }
    
    nonisolated func fetchMinimums(forItem item_natural_key: String) async throws -> [ItemMinimumModel] {
        return minimums.values.filter { $0.item_natural_key == item_natural_key }
    }
    
    nonisolated func fetchMinimums(forStore store: String) async throws -> [ItemMinimumModel] {
        return minimums.values.filter { $0.store == store }
    }
    
    nonisolated func createMinimum(_ minimum: ItemMinimumModel) async throws -> ItemMinimumModel {
        let key = "\(minimum.item_natural_key)-\(minimum.type)"
        minimums[key] = minimum
        return minimum
    }
    
    nonisolated func createMinimums(_ minimums: [ItemMinimumModel]) async throws -> [ItemMinimumModel] {
    ...
    nonisolated func updateMinimum(_ minimum: ItemMinimumModel) async throws -> ItemMinimumModel {
        let key = "\(minimum.item_natural_key)-\(minimum.type)"
        minimums[key] = minimum
        return minimum
    }
    
    nonisolated func deleteMinimum(forItem item_natural_key: String, type: String) async throws {
        let key = "\(item_natural_key)-\(type)"
        minimums.removeValue(forKey: key)
    }
    
    nonisolated func deleteMinimums(forItem item_natural_key: String) async throws {
    ...
    nonisolated func deleteMinimums(forStore store: String) async throws {
        let keysToRemove = minimums.keys.filter { key in
    ...
    nonisolated func generateShoppingList(forStore store: String, currentInventory: [String: [String: Double]]) async throws -> [ShoppingListItemModel] {
    ...
    nonisolated func generateShoppingLists(currentInventory: [String: [String: Double]]) async throws -> [String: [ShoppingListItemModel]] {
    ...
    nonisolated func getLowStockItems(currentInventory: [String: [String: Double]]) async throws -> [LowStockItemModel] {
    ...
    nonisolated func setMinimumQuantity(_ quantity: Double, forItem item_natural_key: String, type: String, store: String) async throws -> ItemMinimumModel {
        let key = "\(item_natural_key)-\(type)"
    ...
    nonisolated func getDistinctStores() async throws -> [String] {
        return Array(Set(minimums.values.map { $0.store })).sorted()
    }
    
    nonisolated func getStores(withPrefix prefix: String) async throws -> [String] {
        let stores = try await getDistinctStores()
        return stores.filter { $0.hasPrefix(prefix) }
    }
    
    nonisolated func getStoreUtilization() async throws -> [String: Int] {
        return Dictionary(grouping: minimums.values, by: { $0.store })
            .mapValues { $0.count }
    }
    
    nonisolated func updateStoreName(from oldStoreName: String, to newStoreName: String) async throws {
    ...
    nonisolated func getMinimumQuantityStatistics() async throws -> MinimumQuantityStatistics {
        return MinimumQuantityStatistics(minimums: Array(minimums.values))
    }
    
    nonisolated func getHighestMinimums(limit: Int) async throws -> [ItemMinimumModel] {
        return Array(minimums.values)
            .sorted { $0.quantity > $1.quantity }
            .prefix(limit)
            .map { $0 }
    ...
    nonisolated func getMostCommonTypes() async throws -> [String: Int] {
        return Dictionary(grouping: minimums.values, by: { $0.type })
            .mapValues { $0.count }
    }
    
    nonisolated func validateMinimumRecords(validItemKeys: Set<String>) async throws -> [ItemMinimumModel] {
