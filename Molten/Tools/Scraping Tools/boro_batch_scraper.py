...
def fetch_product_description(product_url, product_name):
    ...
    try:
        full_url = product_url if product_url.startswith('http') else f"https://store.borobatch.com{product_url}"
...
def determine_product_type(product_name, product_type_field=None):
    ...
    if product_type_field:
        ...
        if 'frit' in type_lower or 'powder' in type_lower:
            ...
            return 'rod'  # Boro Batch "Thins" are thin rods
...
def scrape_boro_batch_products(collection_handle='color-rods', test_mode=False):
    """
    Scrapes Boro Batch products using Shopify's JSON API
    
    Args:
        collection_handle: The collection handle (e.g., 'color-rods', 'all')
        test_mode: If True, only scrape the first item
    """
    print(f"\nScraping Boro Batch collection: {collection_handle}")
    ...
    while True:
        ...
        try:
            ...
            for product_data in products:
                ...
                # Skip products with "+" in the name (combo packs)
                if '+' in product_name:
                    print(f"    Skipping product with '+': {product_name}")
                    continue
                
                # Skip "Bag of Bits" or similar
                if 'bag of bits' in product_name.lower() or 'scrap' in product_name.lower():
                    print(f"    Skipping scrap/bits: {product_name}")
                    continue
                ...
                # Skip non-glass items (stickers, apparel, etc.)
                if any(term in product_name.lower() for term in ['sticker', 'shirt', 't-shirt', 'hoodie', 'hat', 'cap']):
                    print(f"    Skipping non-glass item: {product_name}")
                    continue
                ...
                if not product.get('sku'):
                    name = product['name']
                    # Try BB-XXX format
                    sku_match = re.search(r'^BB-(\d+[A-Za-z]?)', name, re.IGNORECASE)
                    if sku_match:
                        product['sku'] = sku_match.group(1)
                    else:
                        # Try just numbers at start
                        sku_match = re.search(r'^(\d{2,4}[A-Za-z]?)\s+[-–]', name)
                        if sku_match:
                            product['sku'] = sku_match.group(1)
...
def main():
    ...
    print("Starting scrape of Boro Batch glass products...")
    print("=" * 60)
    
    # You can specify different collection handles here
    # Common collections might be: 'color-rods', 'all', 'thins', 'production-colors', etc.
    collections = [
        'color-rods',
    ...
    for collection_handle in collections:
        products, duplicates = scrape_boro_batch_products(
    ...
    try:
        import csv
        csv_filename = 'boro_batch_products_test.csv' if test_mode else 'boro_batch_products.csv'
        ...
        with open(csv_filename, 'w', newline='', encoding='utf-8') as f:
            ...
            for product in all_products:
                ...
                writer.writerow({
                    ...
                    'tags': tags,
                    'synonyms': '',
                    'coe': '33',  # Boro Batch is COE 33 (borosilicate)
                    'type': product_type,
