#!/usr/bin/env python3
"""
Kashmir Glass Scraper
====================

Scrapes Kashmir Glass from ABR Imagery (abrimagery.com).
Platform: Shopify
...
# Constants
MANUFACTURER_CODE = 'KAS'
MANUFACTURER_NAME = 'Kashmir Glass'
COE = '104'  # Kashmir is COE 104
...
def determine_product_type(product_name, tags):
    ...
    if 'frit' in name_lower or 'frit' in tags_lower:
        ...
        return 'rod'  # Default to rod for Kashmir


def remove_brand_from_title(title):
    """Remove Kashmir brand name and product type from title"""
    cleaned_title = title

    # Remove "Kashmir" prefix and "Rod/Tube" suffix
    cleaned_title = re.sub(r'^Kashmir\s+(Rod|Tube|Frit)\s*[-–]?\s*', '', cleaned_title, flags=re.IGNORECASE)

    # Remove brand patterns
    brand_patterns = ['Kashmir Glass', 'Kashmir']
...
def scrape(test_mode=False, max_items=None):
    """
    Scrape Kashmir Glass products from ABR Imagery using Shopify API.
    ...
    while True:
        ...
        try:
            ...
            for product_data in products:
                # Only process Kashmir Glass products
                tags = product_data.get('tags', [])
                if 'Kashmir' not in tags:
                    continue
            ...
            print(f"    Found {kashmir_count} Kashmir Glass products on this page")
    ...
    print(f"\n  Total Kashmir Glass products scraped: {len(all_products)}")
    if duplicates:
