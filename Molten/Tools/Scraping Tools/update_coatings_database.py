#!/usr/bin/env python3
"""
Git+JSON Database Updater for Coating Products
===============================================

This script maintains a version-controlled JSON database of coating products:
- Runs scrapers to get latest data from manufacturer websites
- Filters out excluded URLs (see excluded_coating_urls.txt)
- Applies SKU overrides for products with missing/incorrect codes (see coating_sku_overrides.txt)
...
Usage:
    python3 update_coatings_database.py                    # Update from all manufacturers
    python3 update_coatings_database.py --test             # Test mode (2-3 items)
    python3 update_coatings_database.py --mfr JET          # Update single manufacturer
    python3 update_coatings_database.py --dry-run          # Show changes without saving
    python3 update_coatings_database.py --auto-commit      # Automatically commit to Git

URL Exclusion:
    Edit excluded_coating_urls.txt to add product URLs that should never be included
    (e.g., sample packs, gift sets, non-individual products)

SKU Overrides:
    Edit coating_sku_overrides.txt to fix products with missing or incorrect SKU codes
...
import combined_coatings_scraper


# Database schema version (increment when schema changes)
DATABASE_VERSION = "1.0"
DATABASE_FILE = "coatings_database.json"
EXCLUDED_URLS_FILE = "excluded_coating_urls.txt"
SKU_OVERRIDES_FILE = "coating_sku_overrides.txt"
...
def main():
    ...
    parser.add_argument('--mfr', choices=list(combined_coatings_scraper.MANUFACTURERS.keys()),
    ...
    success = combined_coatings_scraper.main(scraper_args)
